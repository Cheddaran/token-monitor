...
A desktop widget that shows live token usage and AI Tool Limits across 34+ AI coding tools — Claude Code, Codex, Cursor, GitHub Copilot, Cherry Studio, and more — with real-time multi-device sync, historical usage trends, and breakdowns by tool, device, model, session, or project.
...
| <img src=".github/assets/tools-icon/mcode.png" width="28" alt="MiniMax Code" /> | MiniMax Code | Desktop app (`~/.minimax/v2/sessions/`), Tokscale headless capture (`~/.config/tokscale/headless/mcode/` via `tokscale headless mcode`) | ✅ | — | — |
...
- **AI Tool Limits detection** — provider-specific session, daily, weekly, billing, and credits windows for Claude Code, Codex, Cursor, OpenRouter, third-party APIs, GLM, Kimi, and 22+ providers, including multiple OpenRouter/third-party profiles and DeepSeek prepaid balance/spend
