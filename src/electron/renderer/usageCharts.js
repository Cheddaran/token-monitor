...
(function exposeUsageCharts(root, factory) {
  ...
  const clientColors = {
    ...
    pi: '#000', zed: '#4173e7', kilocode: '#F8F676', commandcode: '#8C4EDD', mcode: '#f23f5d', micode: '#000000', zcode: '#000000', kiro: '#9046FF', codebuddy: '#6C4DFF', workbuddy: '#0DC8A5', proma: '#000000', qodercn: '#2ADB5C', reasonix: '#4d6bfe', dsh: '#4d6bfe', cherrystudio: '#EA5E5D', lmstudio: '#6C5CE7', unsloth: '#40B85A',
  ...
  function heatmapSvg(model, options) {
    ...
    return `<svg class="dash-heatmap" viewBox="0 0 ${svgRound(modelWidth)} ${svgRound(modelHeight + botPad)}" width="${svgRound(modelWidth)}" height="${svgRound(modelHeight + botPad)}">${defs}<g class="heat-base-layer">${cells}</g>${brightLayer}${months}</svg>`;
  }
